...
r2 = 0.1683;%m outer r 
A_tube =2*pi*r1; %m^2
h_wall = k_PEH/(r2-r1);%W/m^2*k 
SA_tubes  = r2*2*pi*L_tubes;%total all tube length
SA_tube = r2*2*pi*L_tube; %1 section of tube
burial_D = 3.9624;% 13 feet
%Air properties
%https://en.wikipedia.org/wiki/Heat_transfer_coefficient#Internal_flow,_turbulent_flow
%^^ SEE INTERNAL, TURBULENT FLOW & HEAT TRANSFER COEFFICIENT OF PIPE WALL
V = 33454.7; %ft^3
V = V*0.0283168; %m^3
Cp = 1006; %J/kg*K
rho = 1.225; %0.6601; %@6000 kg/m^3
CFM = (30.6+35.7)/2; %ft^3/min CFM per tube(averaged for the two subsystems)
V_m3 = CFM*0.00047194745;% m^3/s %volumetric flow rate per tube 
v_in = V_m3/A_tube; %m/s
m_dot = V_m3*rho; %kg/s
k_air = .02514;%@20C @ 1atm W/m*k 
mu_air = 1.825*10^-5; %dynamic viscosity
n = .33; %.4 for wall hotter than the bulk fluid, .33 for wall cooler than the bulk fluid
h_air = .023*((m_dot*r1*2/mu_air)^.8)*(((mu_air*Cp)/k_air)^n)*(k_air/r1*2);
h_pipe = 1/((1/h_air)+ (1/h_wall));
%Dirt properties
...
T_delta3 = q *R_T*SA_tube*(1); %(7 + 8) is the number of tubes within each system
...
x = burial_D;
t = 1:7.776e+6; %90 days in seconds (approximate length of winter)
...
for i=1:length(t)
    if i == 1 
        ...
        if  T_EarthPW(i) < t1w*1.01 && T_EarthPW(i) > t1w && flag==0
            t_end = t(i)/(60*60*24);
            disp([num2str(t_end),' Days to reach ambient Earth temperature'])
            flag =1;
        end
    end
end
